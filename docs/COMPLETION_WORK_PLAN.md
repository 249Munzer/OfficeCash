...
- [ ] `npm run build` → نجاح بلا تحذير chunk
- [ ] `npm test` → أخضر (بعد P2.1)
- [ ] لا نصوص مشوّهة (`rg "â€|âœ"` = 0)
- [ ] تبديل الموظف يتطلب PIN
- [ ] مفتاح القفل التلقائي يعمل من الإعدادات
- [ ] `package.json` مكتمل (description/author/license)
- [ ] أول commit نظيف في Git
